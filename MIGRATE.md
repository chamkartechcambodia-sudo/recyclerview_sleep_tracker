...
> **Why `disallowKotlinSourceSets=false`:** KSP adds its generated sources
> (`build/generated/ksp/...`) via `kotlin.sourceSets` DSL. AGP 9.0.1 blocks this by default,
> causing: *"Using kotlin.sourceSets DSL to add Kotlin sources is not allowed with built-in Kotlin"*.
> This flag is **required whenever the `ksp` plugin is used** (Room, Hilt, Moshi codegen).
> Safe to include in all projects — has no effect when KSP is not present.
...
| `Using kotlin.sourceSets DSL to add Kotlin sources is not allowed with built-in Kotlin` | KSP adds generated sources via `kotlin.sourceSets`, blocked by AGP 9.0.1 | Add `android.disallowKotlinSourceSets=false` to `gradle.properties` |
...
□ Updated: gradle.properties → AndroidX, nonTransitiveRClass, disallowKotlinSourceSets=false
...
□ Verified: android.disallowKotlinSourceSets=false in gradle.properties (required when ksp is used)
